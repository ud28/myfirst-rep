...
/**
 * Destroys all FormattedTextViews of the RichTooltip
 * 
 * @public
 */
sap.ui.commons.RichTooltip.prototype.exit = function() {
	if (this._oText) {
		this._oText.destroy();
		delete this._oText;
	}

	if (this._oValueStateText) {
		this._oValueStateText.destroy();
		delete this._oValueStateText;
	}
};
...
sap.ui.commons.RichTooltip.prototype.onAfterRendering = function() {
	if (this._oText && this._oText.getDomRef()) {
		// set a corresponding ARIA role if there is a text
		this._oText.$().attr("role", "tooltip");
...
sap.ui.commons.RichTooltip.prototype.setValueStateText = function(sText) {
	if (sText) {
		if (this._oValueStateText) {
			this._oValueStateText.setProperty("htmlText", sText, true);
		} else {
			this._oValueStateText = new sap.ui.commons.FormattedTextView(this.getId() + "-valueStateText", {
				htmlText : sText
...
sap.ui.commons.RichTooltip.prototype.getValueStateText = function() {
	if (this._oValueStateText) {
		return this._oValueStateText.getHtmlText();
	}
	return "";
};
...
sap.ui.commons.RichTooltip.prototype.setText = function(sText) {
	if (this._oText) {
		this._oText.setProperty("htmlText", sText, true);
	} else {
		this._oText = new sap.ui.commons.FormattedTextView(this.getId() + "-txt", {
			htmlText : sText
...
sap.ui.commons.RichTooltip.prototype.getText = function() {
	if (this._oText) {
		return this._oText.getHtmlText();
	}
	return "";
};
