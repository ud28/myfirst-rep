...
//sap.ui.core.EnabledPropagator.call(sap.ui.commons.form.FormContainer.prototype);
		...
		if (bExpandable) {
			...
			if (!this._oExpandButton) {
				if (!jQuery.browser.mobile) {
					...
					this._oExpandButton = new sap.m.Button(this.getId()+"--Exp",{
//						lite: true
					}).setParent(this);
					this._oExpandButton.attachEvent('tap', handleExpButtonPress, that); // attach event this way to have the right this-reference in handler
				}
			}
	...
	function _setExpanderIcon(oContainer){
		...
		if (oContainer._oExpandButton.setIconHovered) {
			oContainer._oExpandButton.setIconHovered(sIconHovered);
		}
		if (oContainer._oExpandButton.setactiveIcon) {
			oContainer._oExpandButton.setactiveIcon(sIconHovered);
		}
